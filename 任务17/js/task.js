/* 数据格式演示
var aqiSourceData = {
  "北京": {
    "2016-01-01": 10,
    "2016-01-02": 10,
    "2016-01-03": 10,
    "2016-01-04": 10
  }
};
*/

// 以下两个函数用于随机模拟生成测试数据
function getDateStr(dat) {
  var y = dat.getFullYear();
  var m = dat.getMonth() + 1;
  m = m < 10 ? '0' + m : m;
  var d = dat.getDate();
  d = d < 10 ? '0' + d : d;
  return y + '-' + m + '-' + d;
}
function randomBuildData(seed) {
  var returnData = {};
  var dat = new Date("2016-01-01");
  var datStr = '';
  for (var i = 1; i < 92; i++) {
    datStr = getDateStr(dat);
    returnData[datStr] = Math.ceil(Math.random() * seed);
    dat.setDate(dat.getDate() + 1);
  }
  return returnData;
}

var aqiSourceData = {
  "北京": randomBuildData(500),
  "上海": randomBuildData(300),
  "广州": randomBuildData(200),
  "深圳": randomBuildData(100),
  "成都": randomBuildData(300),
  "西安": randomBuildData(500),
  "福州": randomBuildData(100),
  "厦门": randomBuildData(100),
  "沈阳": randomBuildData(500)
};

// 用于渲染图表的数据
var chartData = {};

// 记录当前页面的表单选项
var pageState = {
  nowSelectCity: "北京",
  nowGraTime: "day"
}

/**
 * 渲染图表
 */
function renderChart() {
	var aqiChart = document.getElementsByClassName("aqi-chart-wrap");
	var str = "";
	for(var v in chartData){
		str += "<div class='list "+pageState["nowGraTime"]+"'>";
		str += "<div class='high' style='height:"+chartData[v]+"px;background-color:"+getRandomColor()+"' title='"+v+":"+chartData[v]+"'></div>";
		str += "</div>";
	}
	aqiChart[0].innerHTML = str;
}
function getRandomColor(){
	return "#"+Math.floor(Math.random()*16).toString(16)+Math.floor(Math.random()*16).toString(16)
	+Math.floor(Math.random()*16).toString(16)+Math.floor(Math.random()*16).toString(16)+Math.floor(Math.random()*16).toString(16)
	+Math.floor(Math.random()*16).toString(16);
}
/**
 * 日、周、月的radio事件点击时的处理函数
 */
function graTimeChange() {58
  // 确定是否选项发生了变化 
var typeNow = getTimeNow();
if(typeNow==pageState["nowGraTime"]){
	return 0;
}else{
	//pageState[nowGraTime]=typeNow;

  // 设置对应数据,重新获取chartData
	initAqiChartData();
  // 调用图表渲染函数
  renderChart();
 }
}
function getTimeNow(){
	var timeList = document.getElementsByName("gra-time");
	var typeNow="";
	for(var i=0;i<timeList.length;i++){
		if(timeList[i].checked){
			typeNow = timeList[i].value;		
		}
	}
//	[].forEach.call(timeList,function(v){
//		if(v.checked){
//			typeNow = v.value;
//		}
//	});
	return typeNow;
}
/**
 * select发生变化时的处理函数
 */
function citySelectChange() {
  // 确定是否选项发生了变化 
	var cityNow = document.getElementById("city-select").value;
	if(cityNow==pageState["nowSelectCity"]){
		return;
	}else{
		initAqiChartData(); // 设置对应数据
		renderChart();// 调用图表渲染函数
	}
}

/**
 * 初始化日、周、月的radio事件，当点击时，调用函数graTimeChange
 */
function initGraTimeForm() {
	var timeList = document.getElementsByName("gra-time");
//	for(var i=0;i<timeList.length;i++){
//		timeList[i].addEventListener("click",graTimeChange);
//	}
	[].forEach.call(timeList,function(value){
		value.addEventListener("click",graTimeChange);
	});
}

/**
 * 初始化城市Select下拉选择框中的选项
 */
function initCitySelector() {
  // 读取aqiSourceData中的城市，然后设置id为city-select的下拉列表中的选项
	var citySelect = document.getElementById("city-select");
	var cityList = "";
	for(var v in aqiSourceData){
		cityList += "<option value='"+v+"'>"+v+"</option>";
	}
	citySelect.innerHTML = cityList;
  // 给select设置事件，当选项发生变化时调用函数citySelectChange
  citySelect.addEventListener("change",citySelectChange);
  
}

/**
 * 初始化图表需要的数据格式
 */
function initAqiChartData() {
  // 将原始的源数据处理成图表需要的数据格式
  // 处理好的数据存到 chartData 中
  var type = getTimeNow();
  var city = document.getElementById("city-select").value;
  pageState["nowSelectCity"] = city;
  pageState["nowGraTime"] = type;
  switch(type){
  	case "day":
  		chartData = aqiSourceData[city];
  		break;
  	case "week":
  		chartData={};
  		var chartData1 = aqiSourceData[city];
  		var week=1 ,sum=0, count=0;
  		for(var v in chartData1){
  			var date = new Date(v);
  			var dat = date.getDay();
  			if(dat == 6){		
  				count++;
  				sum+=chartData1[v];
  				chartData["week"+week]=Math.round(sum/count); 	
  				week++;
  				sum=0;
  				count=0;
  			}else{
  				count++;
  				sum+=chartData1[v];  				
  			}
  		}
  		chartData['week'+week]=Math.round(sum/count);
  		break;
  	case "month":
  		chartData={};
  		var chartData1 = aqiSourceData[city];
  		var month=-1,sum=0,count=0;
  		for(var v in chartData1){
  			var date = new Date(v);
  			if(month==-1){
  				month=date.getMonth()+1;
  			}else if(date.getMonth()+1 != month){
  				chartData["month"+month]=Math.round(sum/count); 				
  				count=1;
  				sum=chartData1[v];
  				month=date.getMonth()+1;
  			}else{
  				sum+=chartData1[v];
  				count++;
  			}
  		}
  		chartData["month"+month]=Math.round(sum/count);
  		break;
  		
  		
  }
  renderChart();
}

/**
 * 初始化函数
 */
function init() {
  initGraTimeForm()
  initCitySelector();
  initAqiChartData();
}
window.onload=function(){
	init();
}
